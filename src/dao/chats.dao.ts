...
import { Message } from "../custom-types/dtos/message.dto.type";
...
async function getConversationByChatId(
  ...
  const otherUser = await db("chats")
    ...
    .select(
      db.raw(
        "case when chats.user1_id != ? then u1.full_name else u2.full_name end as name",
        [userId]
      ),
      db.raw(
        "case when chats.user1_id != ? then os1.is_online else os2.is_online end as is_online",
        [userId]
      ),
      db.raw(
        "case when chats.user1_id != ? then os1.last_seen else os2.last_seen end as last_seen",
        [userId]
      )
    )
  ...
  const messages = await db("messages")
    .select(
      db.raw(
        "case when messages.sender_id = ? then true else false end as is_me",
        [userId]
      ),
